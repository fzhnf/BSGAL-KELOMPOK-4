#!/usr/bin/env python3
"""
BSGAL + Classifier-Balancing  —  Notebook Generator
====================================================

Run this script to produce ``bsgal.ipynb``::

    uv run python bsgal.py

Requirements: ``nbformat`` (auto-installed if missing).
The generated notebook contains the full BSGAL training pipeline enhanced
with four improvements from *Decoupling Representation and Classifier for
Long-Tailed Recognition* (Kang et al., ICLR 2020):

  - cRT (Classifier Re-Training with class-balanced sampling)
  - τ-normalization (post-hoc weight rescaling)
  - LWS (Learnable Weight Scaling)
  - Class-aware gradient analysis in BSGAL decisions

Sections 1–12 match the original ``bsgal.py`` / ``bsgal.ipynb``.
Sections 13–16 are new (classifier-balancing additions).
Sections 17–25 are the training loops, evaluation, and visualisation.
"""
...
# Silenced pyright "possibly unbound" warnings — these are bound inside the
# for-loop below before _build_notebook() is ever called.
new_notebook = None  # pylint: disable=invalid-name
new_markdown_cell = None  # pylint: disable=invalid-name
new_code_cell = None  # pylint: disable=invalid-name
nbf = None
for _ in range(1):
    try:
        import nbformat as nbf
    except ImportError:
        print("[setup] installing nbformat …")
        subprocess.check_call([sys.executable, "-m", "pip", "install", "nbformat"])
        try:
            import nbformat as nbf
        except ImportError:
            # GitHub Codespaces / restrictive environments
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "--user", "nbformat"]
            )
            import nbformat as nbf

    # nbformat writes notebook format 4 (the current standard)
    from nbformat.v4 import new_notebook, new_markdown_cell, new_code_cell


# ════════════════════════════════════════════════════════════════════════════
#  Cell content — each "cell" is a (type, source_string) tuple.
#  Markdown cells use raw strings; code cells keep the exact code the
#  student will run inside the notebook.
# ════════════════════════════════════════════════════════════════════════════
...
def M(text: str):
    """Append a markdown cell (dedented)."""
    CELLS.append((_md, textwrap.dedent(text).strip()))


def C(text: str):
    """Append a code cell (dedented)."""
    CELLS.append((_cd, textwrap.dedent(text).strip()))
...
if cfg.TINY:
    cfg.BASELINE_EPOCHS = 3          # enough iters for RPN + heads to converge
    cfg.BSGAL_EPOCHS    = 2          # BSGAL is expensive — keep at 2
    cfg.CRT_EPOCHS      = 3          # classifier re-training benefits from more epochs
    cfg.LWS_EPOCHS      = 2
    cfg.TRAIN_SUBSET_SIZE = 500      # key change: 64→500 (×8 more data)
    cfg.VAL_SUBSET_SIZE   = 48       # 8→48 for stable eval
...
M("## 7. Synthetic Instance Pool (Copy-Paste)")
...
M("## 10. Repeat-Factor Sampler (Class-Balanced)")
...
# ─── 15. BSGAL GRADIENT BANK (ORIGINAL) ────────────────────────────────────
...
# ── Evaluate ALL checkpoints (three-way comparison) ─────────────────────────
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="BSGAL + CB notebook generator")
    parser.add_argument(
        "-o",
        "--output",
        default="bsgal.ipynb",
        help="Output .ipynb path (default: bsgal.ipynb)",
    )

...
# Install dependencies (both Colab and local)
...
class Config:
    ...
    TRAIN_SUBSET_SIZE: Optional[int] = 2000  # None = full LVIS train
    VAL_SUBSET_SIZE: Optional[int] = 200     # None = full val
    IMS_PER_BATCH: int = 4
    ...
    # ─── Logging & Checkpoint ───────────────────────────
    LOG_EVERY: int = 50
    EVAL_EVERY_EPOCH: int = 1     # 0 = skip mid-training eval
    ...
    BSGAL_WARMUP_ITERS: int = 200  # shorter for fine-tune phase
...
def evaluate_on_lvis(
    ...
    eval_inst.run()
    # Don't print here — caller decides.
    out = {}
...
def make_baseline_loader():
    ...
    return DataLoader(
        ...
        num_workers=cfg.NUM_WORKERS, collate_fn=collator, pin_memory=True, persistent_workers=cfg.NUM_WORKERS > 0,
    )
...
_dbg_loader = make_triplet_loader()
_dbg_iter = iter(_dbg_loader)
_dbg_triplet = next(_dbg_iter)
del _dbg_iter, _dbg_loader
